...
{
    private static readonly string[] _classNameSuffixes = ["Async", "Impl", "Manager", "Helper", "Service"];
    private static readonly string[] _staticClassNameSuffixes = ["Extensions", "Converters", "Ids", "WellKnown", "Defaults"];
    private static readonly ImmutableHashSet<string> _builtInExceptions =
    [
        "System.Exception",
    ...
    private static readonly DiagnosticDescriptor _rule0001 = CreateRule("CRARCH0001", "Exception type naming", "Exception type '{0}' must not end with 'Exception'", "Rename exception types to domain terms without the Exception suffix, for example AuthorNotFound instead of AuthorNotFoundException.");
    private static readonly DiagnosticDescriptor _rule0002 = CreateRule("CRARCH0002", "No built-in exception types", "Throw custom domain exceptions instead of '{0}'", "Replace thrown framework exceptions with domain-specific exception types that express business intent.");
    ...
    private static readonly DiagnosticDescriptor _rule0010 = CreateRule("CRARCH0010", "Constructor fan-out", "Constructor has {0} dependencies (maximum is 7)", "Reduce constructor dependencies to seven or fewer by splitting responsibilities or introducing a more focused abstraction.");
    private static readonly DiagnosticDescriptor _rule0011 = CreateRule("CRARCH0011", "File length threshold", "File has {0} effective lines (maximum is 400)", "Split large files into smaller focused types once effective code lines exceed 400.");
    private static readonly DiagnosticDescriptor _rule0012 = CreateRule("CRARCH0012", "async void forbidden", "Avoid async void methods outside event handlers", "Change async void methods to async Task unless the method is an event handler.", DiagnosticSeverity.Error);
    private static readonly DiagnosticDescriptor _rule0013 = CreateRule("CRARCH0013", "No blocking on async", "Avoid blocking async calls via .Result, .Wait(), or .GetAwaiter().GetResult()", "Replace synchronous blocking on asynchronous operations with await to avoid deadlocks and hidden failures.");
    private static readonly DiagnosticDescriptor _rule0014 = CreateRule("CRARCH0014", "No test types in production", "Production code must not reference testing/specification types: '{0}'", "Remove references to .Specs/.Testing types from production code and replace them with production abstractions.", DiagnosticSeverity.Error);
    private static readonly DiagnosticDescriptor _rule0015 = CreateRule("CRARCH0015", "Static class naming convention", "Static class '{0}' must end with one of: {1}", "Rename static classes to one of the approved suffixes: Extensions, Converters, Ids, WellKnown, or Defaults.");
    private static readonly DiagnosticDescriptor _rule0016 = CreateRule("CRARCH0016", "Unused interfaces", "Interface '{0}' has no concrete implementations", "Remove speculative interfaces with no implementations or add a concrete implementation where the abstraction is used.");
    private static readonly DiagnosticDescriptor _rule0017 = CreateRule("CRARCH0017", "Namespace must align with folder path", "Namespace '{0}' should align with folder path '{1}'", "Adjust the declared namespace or move the file so namespace segments match the folder structure.");
    ...
    private static DiagnosticDescriptor CreateRule(string id, string title, string messageFormat, string description, DiagnosticSeverity severity = DiagnosticSeverity.Warning)
